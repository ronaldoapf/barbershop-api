...
### 2026-08-17 — Add `BarberService` capability table

Migration `20260817041810_add_barber_services_table` applied. New model `BarberService` (`barberId`, `serviceId`, `@@unique([barberId, serviceId])`), with inverse relations added on `Barber` and `Service`. `spec-en.md` never modeled which services a given barber can perform — the catalog is barbershop-wide and commission is a flat per-barber percentage, so nothing constrained booking a barber for a service they don't actually offer. This join table makes that an explicit, enforced relationship: `plan.md` decision #7 makes it a hard block in the booking flow (Phase 8) — `get-available-slots`/`create-appointment` reject any requested service not assigned to the chosen barber. New barbers start with zero assigned services (explicit opt-in by `OWNER`). Unlike every other delete in this codebase, unassigning a service from a barber is a **physical delete** of the join row, not a soft delete — `BarberService` is a pure current-state relation with no standalone business history worth preserving (unlike `User`/`Service`/`Appointment`), so the soft-delete rule doesn't apply to it.

### 2026-08-17 — `POST /services` accepts optional `barberIds`

No schema change. `CreateServiceDto`/`CreateServiceUseCase` gained an optional `barberIds: string[]`; when present, `ServicesRepository.create()` creates the `Service` and all its `BarberService` assignments (deduped via `Set`, `barberService.createMany`) in one `$transaction`, so `OWNER` can assign every capable barber without N separate calls to `POST /barbers/:barberId/services`. Deliberately implemented inside `services.repository.ts` rather than by injecting `IBarberServicesRepository` into `CreateServiceUseCase`, because `BarberServicesModule` already imports `ServicesModule` (to validate a service exists before assigning it) — importing back would create a Nest module cycle. Any invalid `barberId` in the array surfaces as `404` via a `P2003` (foreign-key violation) catch, since the global `DatabaseExceptionFilter` doesn't map that code by default.

### 2026-08-17 — Add commission snapshot fields to `AppointmentService`

Migration `20260817045349_add_appointment_commission_fields` applied. New nullable fields `commissionPercentageApplied Decimal? @db.Decimal(5,2)` and `commissionAmount Int?` on `AppointmentService`, per `plan.md` decision #1: commission can't be a read-time computation off `Barber.commissionPercentage`, since that rate changes over time and recomputing from the current rate would silently rewrite historical reports. Both fields stay `null` until `complete-appointment.use-case.ts` (Phase 8) freezes them at completion time — mirroring the existing `serviceName`/`price`/`durationMinutes` snapshot pattern already on this table. `commissionAmount` is `0` when an item is `redeemedWithPoints` and `Setting['commission_on_redeemed_service']` is false (or absent — the setting defaults closed), resolving spec §4.3/§6's open question explicitly rather than leaving it inferred.

### 2026-08-18 — Make `User.email` optional, add unique constraint on `User.phone`

Migration `20260818031130_make_user_email_optional_phone_unique` applied. `email String? @unique` (was `String @unique`), `phone String? @unique` (was `String?`, no constraint). Surfaced while building Phase 10 (WhatsApp-facing endpoints): `spec-en.md` §4.5 says a WhatsApp customer is "identified only by verified phone number, no password required," and the chatbot "automatically creat[es] the `CUSTOMER` `User` on first interaction if needed" — but the original schema made `email` mandatory and put no uniqueness constraint on `phone`, so a phone-only customer couldn't be created at all, and a find-or-create-by-phone flow would have had a race condition (two concurrent lookups could both see "no match" and insert duplicate customers for the same number). No other flow is affected: `register`/`login`/Google OAuth/barber invites all still supply a real `email` at creation time, so `User.email` being nullable at the schema level doesn't change their behavior — it only unblocks the phone-only path. Chose this over generating a synthetic placeholder email for WhatsApp customers, which would have left fabricated data sitting in the `email` column.

### 2026-08-18 — Phase 10: WhatsApp-facing endpoints

No further schema change. Added `find-or-create-customer-by-phone.use-case.ts` to the `users` module (looks up by `phone`; on the create path, a `P2002` race from a concurrent duplicate request is treated as success — it re-fetches by phone instead of throwing, since "auto-create on first interaction" is meant to be idempotent, not a hard conflict). Added `src/shared/guards/service-api-key.guard.ts` (validates a static `x-api-key` header against `WHATSAPP_SERVICE_API_KEY`, per `spec-en.md` §2's `ServiceApiKeyGuard`). Two thin controllers reuse existing use cases unchanged except for one addition: `whatsapp-users.controller.ts` (`POST /whatsapp/customers`) and `whatsapp-appointments.controller.ts` (`POST /whatsapp/appointments`, `GET /whatsapp/available-slots`), both guarded by `ServiceApiKeyGuard` only (no `JwtAuthGuard`/`RolesGuard`, since the caller is the external chatbot service, not a platform user) — kept as separate controller classes rather than new methods on `UsersController`/`AppointmentsController`, since NestJS guards stack rather than override, so a method added to those class-guarded controllers couldn't drop the class-level `JwtAuthGuard` requirement. `CreateAppointmentUseCase` gained an optional `source` field on its input (defaulting to `AppointmentSource.PLATFORM`) so the WhatsApp controller can tag bookings `AppointmentSource.WHATSAPP` instead of every booking silently reporting `PLATFORM` regardless of origin — the enum value already existed on `AppointmentEntity.source` but nothing set it until now.
